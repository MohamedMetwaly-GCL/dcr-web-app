...
async function openAdmin(){{
  ...
  // Remove top padding reset if any
  body.style.paddingTop = '';
  
  // Create static header outside of scrolling body
  let existingUtitle = document.getElementById('admin-utitle');
  if (existingUtitle) existingUtitle.remove();
  
  const utitle=document.createElement('div');
  utitle.id='admin-utitle';
  utitle.style.cssText = 'padding:12px 18px; border-bottom:1px solid var(--bd); background:var(--wh, #fff); display:flex; justify-content:space-between; align-items:center;';
  utitle.innerHTML=`<div><strong style="font-size:13px;color:var(--tx)">👥 Users</strong> <span class="badge" style="margin-left:8px;background:var(--pr);color:#fff">${{users.length}}</span></div> <input type="text" placeholder="Search users by name or email..." style="padding: 4px 8px; font-size: 11px; border: 1px solid var(--bd); border-radius: 4px; width: 250px; font-weight: normal;" oninput="const q=this.value.toLowerCase(); Array.from(document.querySelectorAll('.urow-wrapper')).forEach(c=>{{c.style.display=c.innerText.toLowerCase().includes(q)?'block':'none';}})">`;
  
  body.parentNode.insertBefore(utitle, body);
  for(const u of users){{
    ...
    row.innerHTML=`<span style="flex:1;font-weight:600;min-width:120px">👤 ${{u.username}}</span>
      <input id="email-${{u.username}}" placeholder="Email (e.g. dc@company.com)" value="${{u.email || ''}}" style="flex:1.5; margin-right: 8px; padding: 4px 6px; font-size: 11px; border: 1px solid var(--bd); border-radius: 4px;" oninput="const b=this.parentElement.querySelector('.role-save-btn'); if(b) {{b.style.opacity='1'; b.style.pointerEvents='auto'; b.style.filter='none';}}" onblur="updUsrEmail('${{u.username}}')">
      ${{u.username!=='admin'?`<button class="btn btn-sc btn-sm" onclick="chgPw('${{u.username}}')">🔑 PW</button>
        <button class="btn btn-er btn-sm" style="background:transparent;color:var(--er);border-color:transparent;font-size:14px;padding:0 4px" title="Delete User" onclick="if(confirm('Are you sure you want to delete this user?')) delUsr('${{u.username}}')">🗑️</button>`:
        '<span style="flex:0 0 auto;font-size:10px;color:var(--mu)">(protected)</span>'}}`;
    if(u.username!=='admin'){{
  ...
  rows.forEach((row,idx)=>{{
    ...
    state.cols.forEach(col=>{{
      ...
      if(col.col_type==='url' || key==='fileLocation'){{
        ...
        if(hasValidLink){{
          let eyeSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M1 12s4-8 11-8 11 8 11 8-4 8-11 8-11-8-11-8z"></path><circle cx="12" cy="12" r="3"></circle></svg>`;
          let dlSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 15v4a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2v-4"></path><polyline points="7 10 12 15 17 10"></polyline><line x1="12" y1="15" x2="12" y2="3"></line></svg>`;
          let icons = `<a class="view-file-icon" href="${{url}}" target="_blank" title="${{isFolder ? 'Open Folder' : 'View File'}}">${{eyeSvg}}</a>`;
          if(!isFolder) {{
            let dlUrl = url;
            ...
            icons += `<a class="dl-file-icon" href="${{dlUrl}}" target="_blank" title="Download File">${{dlSvg}}</a>`;
          }}
          td.innerHTML=icons;
        }}
        else {{
          let eyeOffSvg = `<svg xmlns="http://www.w3.org/2000/svg" width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M17.94 17.94A10.07 10.07 0 0 1 12 20c-7 0-11-8-11-8a18.45 18.45 0 0 1 5.06-5.94M9.9 4.24A9.12 9.12 0 0 1 12 4c7 0 11 8 11 8a18.5 18.5 0 0 1-2.16 3.19m-6.72-1.07a3 3 0 1 1-4.24-4.24"></path><line x1="1" y1="1" x2="23" y2="23"></line></svg>`;
          td.innerHTML=`<span title="No File Attached" style="color:#cbd5e1;cursor:not-allowed;opacity:0.5;display:inline-flex;align-items:center;justify-content:center;">${{eyeOffSvg}}</span>`;
        }}
      ...
      if ((key === 'docNo' || col.col_type === 'docno') && row._has_newer_revision) {{
          ...
          td.innerHTML = escapedVal + ' <svg xmlns="http://www.w3.org/2000/svg" width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="margin-left:4px;vertical-align:middle;color:#9ca3af;cursor:help;" title="Superseded"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/><path d="M12 7v5l4 2"/></svg>';
      }} else {{
...
async function buildForm(row,opts={{}}){{
  ...
  for(const col of formCols){{
    ...
    if(col.col_type==='date'){{const inp=document.createElement('input');inp.type='date';inp.id='f-'+key;inp.value=val;grp.appendChild(inp);}}
      ...
      hid.type = 'hidden';
      hid.id = 'f-'+key;
      hid.className = 'json-data-field';
      hid.value = typeof val === 'string' ? val : JSON.stringify(val||[]);
      grp.appendChild(hid);
      
      const wrap = document.createElement('div');
      wrap.id = 'sd-items-editor-' + key;
      wrap.innerHTML=`
        <table class="sd-table" style="width:100%;border-collapse:collapse;margin-top:4px">
          <thead>
            <tr style="border-bottom:1px solid var(--bd);text-align:left;font-size:11px;color:var(--mu)">
              <th style="padding:4px">Item Ref. / DWG No.</th>
              <th style="padding:4px;width:140px">Status</th>
...
function addSdItemRow(key, item={{}}) {{
  ...
  inp.value = item.item_ref || '';
  sel.value = item.item_status || '';
  inp.addEventListener('input', () => syncSdItems(key));
...
function renderNocSummary(rows){{
  ...
  h.push(`<div style="flex:2.5;min-width:280px;background:#fff;border:1px solid var(--bd);border-radius:8px;padding:10px 12px;display:flex;flex-direction:column;justify-content:center;">
    <div style="font-size:10px;font-weight:700;color:var(--mu);text-transform:uppercase;letter-spacing:.4px;margin-bottom:8px;text-align:center;">Final Approval</div>
    <div style="display:flex;justify-content:space-between;align-items:center;">
      <div style="flex:1;text-align:center">
        <div style="font-size:10px;font-weight:700;color:var(--mu);text-transform:uppercase;letter-spacing:.4px">Request value</div>
      ...
      <div style="flex:1;text-align:center">
        <div style="font-size:10px;font-weight:700;color:var(--mu);text-transform:uppercase;letter-spacing:.4px">Agreed Value</div>
...
async function openDistributionMatrix(pid) {{
  ...
  try {{
    ...
    finalSaveBtn.onclick = async () => {{
      ...
      try {{
        const r = await apiFetch(`/api/project/${{pid}}/settings`, {{
          method: 'POST',
        ...
        if(r && r.ok) {{
          ...
          toast((r && r.error) || 'Failed to save', 'er');
        }}
      }} catch(e) {{
